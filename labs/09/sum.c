...
static int sum_vectorized(int n, int *a)
{
	...
	for (int i = 0; i< n/4*4; i+=4){
		__m128i a1 = _mm_loadu_si128(&(a[i]));
		sum = _mm_add_epi32(sum, a1);

	}


	int fnl[4];
	_mm_storeu_si128(fnl,sum);
	int rtn = 	fnl[0]+ fnl[1] + fnl[2] + fnl[3];
...
static int sum_vectorized_unrolled(int n, int *a)
{
	...
	for (int i = 0; i< n/16*16; i+=16){
		__m128i a1 = _mm_loadu_si128(&(a[i]));
		__m128i a2 = _mm_loadu_si128(&(a[i+4]));
		__m128i a3 = _mm_loadu_si128(&(a[i+8]));
		__m128i a4 = _mm_loadu_si128(&(a[i+12]));
		sum = _mm_add_epi32(sum, _mm_add_epi32(a1,_mm_add_epi32(a2,_mm_add_epi32(a3,a4))));
