...
/* First pass of the assembler. You should implement pass_two() first.
   ...
   be valid. Here are some guidelines:
    ...
    5. A line containing only a label is valid. The address of the label should
        be the byte offset of the next instruction, regardless of whether there                                     sdfgadsfasdfadsssssssssssssssssssssssssssssssssssssssssssssssssssssssss
...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    while(fgets(buf, BUF_SIZE, input)) {
        ...
        int temp = add_if_label(input_line,token,byte_offset,symtbl);
        if(temp!=0) {
            if(temp==-1){
                ret_code=-1;
            }
