...
inst_needs_relocation:
	...
	addu $t0,$a0,$0		#load instruction
	srl $t0,$a0,26		#get the opcode for instruction
	addiu $v0,$0,0		#v0 = 0
	beq $t0,2, Needs_reloc  	#check if instruction is a jump or jal
	beq $t0,3, Needs_reloc
	...
	Needs_reloc:			
		addiu $v0,$0,1	#change v0 to 1 if j or jal
		j End_needs_reloc
...
relocate_inst:
	...
	addu $s0,$0,$a0		#save arguments to reload later
	addu $s1,$0,$a1
	addu $s2,$0,$a2
	addu $s3,$0,$a3

	addu $a0,$s3,$0		#find label name for given address in relocation table
	jal symbol_for_addr
	beq $v0,$0,Error
	
	addu $a1,$v0,$0		#find absolute address of label in symbol table
	addu $a0,$s2,$0
	jal addr_for_symbol
	beq $v0,-1,Error
	
	addu $t1,$v0,$0		
	srl $t1,$t1,2		#get rid of botton 2 0's of lable address since addresses are in words
	
	addu $t0,$s0,$0		#replace address of label into instruction
	srl $t0,$t0,26
	sll $t0,$t0,26
	or $v0,$t0,$t1
	# YOUR CODE HERE
