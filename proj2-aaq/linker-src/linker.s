...
write_machine_code_next_inst:
	...
	# YOUR_INSTRUCTIONS_HERE
	addu $a0,$v1,$0		#load arguments for parse_int, v1 = line of instruction 
	addiu $a1,$0,16
	jal parse_int
	addu $s6,$v0,$0  #s6 = instruction as ints
	...
	# YOUR_INSTRUCTIONS_HERE
	addu $a0,$s6,$0
	jal inst_needs_relocation		#check if j or jal
	beq $v0,0,write_machine_code_to_file  #if i don't need relocation
	...
	# YOUR_INSTRUCTIONS_HERE
	addu $a0,$s6,$0		#load arguments for relocation since they get clobbered, s6 = instruction
	addu $a1,$s5,$0		#s5 = byte offset
	addu $a2,$s2,$0		#s2 = symbol table
	addu $a3,$s3,$0		#s3 = relocation table
	jal relocate_inst	
	addu $s6,$v0,$0		#s6 = instruction with relocated address
...
write_machine_code_to_file:
	...
	la $a1,hex_buffer
	addu $a0, $s6, $0
	jal hex_to_str		#change instruction to string
	
	# 7. Increment the byte offset by the appropriate amount:
	# YOUR_INSTRUCTIONS_HERE
	addiu $s5,$s5,4		#when moving to next line, byte offset is +4 since its next word
