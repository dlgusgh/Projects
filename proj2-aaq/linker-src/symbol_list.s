...
addr_for_symbol:
	...
	addu $s0,$a0,$0			# save arguments to call later
	addu $s1,$a1,$0
	addiu $v0,$0,-1			#if not found return -1
	Loop_through_list:
		beq $s0, $0, End_addr_for_symbl			#if end of list, exit
		lw $a3,0($s0)						#load arguments of If_equal_sym
		lw $a0,4($s0)
		addu $a1,$s1,$0
		jal streq
		beq $v0,0, If_equal_sym			#if symbol is found, exit
		lw $s0, 8($s0)					#load next symbol
		j Loop_through_list
	# YOUR CODE HERE
	...
	If_equal_sym:
		addu $v0,$a3,$0						#load v0 with address
		j End_addr_for_symbl
