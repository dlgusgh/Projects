...
unsigned write_pass_one(FILE* output, const char* name, char** args, int num_args) {
    if (strcmp(name, "li") == 0) {
      ...
      long int temp;
      int err = translate_num(&temp, args[1], INT32_MIN, UINT32_MAX);
      if (err!=0){
        return 0;
      }
      if ((temp<=0 && temp>INT16_MIN) || (temp>=0 && temp<INT16_MAX)) {
        fprintf(output, "%s %s %s %d\n", "addiu",args[0],"$0",(int)temp);
        return 1;
      } else {
        int upper =(int) temp>>16;
        int lower =(int) temp&(0xFFFF);
      ...
      if ((temp>INT16_MIN) && (temp<INT16_MAX)) {
        fprintf(output, "%s %s %d %s\n", "lw", args[0], (int)temp, args[2]);
        return 1;
      } else { 
        int upper = ((int) temp>>16)&0xFFFF;
        int lower =(int) temp&(0xFFFF);
...
int translate_inst(FILE* output, const char* name, char** args, size_t num_args, uint32_t addr,
    ...
    else if (strcmp(name, "div") == 0)   return write_mult (0x1a, output, args, num_args);
...
int write_mult(uint8_t funct, FILE* output, char** args, size_t num_args){
  if (num_args==2){
...
int write_rtype(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    int rs = translate_reg(args[1]);
    int rt = translate_reg(args[2]);
    uint32_t instruction = (rd<<11) + (rs<<21) + (rt<<16) + funct;
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_shift(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = (rd<<11) + (rt<<16) + (shamt<<6) + funct;
    write_inst_hex(output, instruction);
    return 0;
}
